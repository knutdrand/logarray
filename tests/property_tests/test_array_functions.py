...
def valid_arrays(draw):
    return draw(stnp.arrays(shape=(4, ), dtype=stnp.integer_dtypes()|stnp.floating_dtypes()))

@given(valid_arrays().filter(lambda a: np.all(a > 0)))
...
def test_sum(regular):
    ...
    assert_logarray_allclose(logarray_sum, regular_sum, rtol=1e-3)
